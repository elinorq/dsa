...
void removeAfter(d_List*& L, int val)
{
    ...
    if (q) { //Nếu có node có key == val
        d_NODE* temp = q->pNext; //Lưu node cần xóa
        if (temp) {
            q->pNext = temp->pNext;
            if (temp->pNext) {
                temp->pNext->pPrev = q;
            }
            else { //Node cần xóa là pTail
                L->pTail = q;
            }
            delete temp;
        }
...
bool addPos(d_List*& L, int data, int pos)
{
    if (pos < 0)
        return false;
    d_NODE* dum = createNode(0);
    dum->pNext = L->pHead;
    d_NODE* q = dum;
    int currPos = 0;

    while (q && currPos < pos) { //di chuyển tới trước vị trí chèn
        currPos++;
        q = q->pNext;
    }

    if (!q) { //pos không tồn tại trong ll
        return false;
    }
...
bool addBefore(d_List*& L, int data, int val)
{
    ...
    while (q && q->pNext && q->pNext->key != val)
        q = q->pNext;
